...
$aColumns = array(
    ...
    't_tickets.telephone',
    't_tickets.email',
    'IFNULL(t_tickets.total_amount, 0) - IFNULL(t_tickets.discount_amount, 0) + IFNULL(t_tickets.total_vat, 0) + IFNULL(t_tickets.lucky_draw_fee, 0) + IFNULL(t_tickets.total_markup, 0)',
    't_tickets.balance',
    't_tickets.type',
    't_tickets.status',
...
if ($_GET['sSearch'] != "") {
    ...
    for ($i = 0; $i < (count($aColumns) - 9); $i++) {
        if($aColumns[$i]  == 't_tickets.code' || $aColumns[$i]  == 'IF(online_orders.code != "", online_orders.code, IF(t_tickets.api_bank_ref != "", t_tickets.api_bank_ref, ""))' || $aColumns[$i] == 'IFNULL(t_tickets.agt_refer_code, "")' || $aColumns[$i] == 't_tickets.telephone' || $aColumns[$i] == 't_tickets.email'){
...
for ($i = 0; $i < (count($aColumns) - 9); $i++) {
    if ($_GET['bSearchable_' . $i] == "true" && $_GET['sSearch_' . $i] != '') {
        ...
        if($aColumns  == 't_tickets.code' || $aColumns[$i]  == 'IF(online_orders.code != "", online_orders.code, IF(t_tickets.api_bank_ref != "", t_tickets.api_bank_ref, ""))' || $aColumns == 'IFNULL(t_tickets.agt_refer_code, "")' || $aColumns == 't_tickets.telephone' || $aColumns[$i] == 't_tickets.email'){
...
while ($aRow = mysql_fetch_array($rResult)) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        } else if ($aColumns[$i] == 'IFNULL(t_tickets.total_amount, 0) - IFNULL(t_tickets.discount_amount, 0) + IFNULL(t_tickets.total_vat, 0) + IFNULL(t_tickets.lucky_draw_fee, 0) + IFNULL(t_tickets.total_markup, 0)') {
            $row[] = number_format($aRow[$i], 2)." ".$aRow[13];
        } else if ($aColumns[$i] == 't_tickets.balance') {
            if($aRow[$i] > 0){
                if($aRow[11] == 2){ // Phone Call
                    $row[] = 'Unpaid';
                } else {
                    $row[] = 'Paid';
                }
    ...
    if($aRow[5] == '0000-00-00 00:00:00' || $aRow[5] == ''){
        ...
        if(checkDateFrom($aRow[15] , $departureTime) == 0) {
    ...
    $row[] =
            ...
            ($allowView && $aRow[12]  > 0 ? '<a href="" class="btnUpdaetNoteTTicket" rel="' . $aRow[0] . '" name="' . $aRow[2] . '" note="' . str_replace('"', "{dblquote}", trim($aRow[17] )) . '"><img alt="Update Note" onmouseover="Tip(\'Update Note\')" src="' . $this->webroot . 'img/button/note.png" /></a> ' : '') .
            ($allowReprint && $aRow[12]  == 2 ? '<a href="" class="btnPrintTTicket" rel="' . $aRow[0] . '" name="' . $aRow[2] . '" print-type="'. $aRow[18] .'"><img alt="Print" onmouseover="Tip(\'' . ACTION_PRINT . '\')" src="' . $this->webroot . 'img/button/printer.png" /></a> ' : '') .
            // ($allowAddLucky && $aRow[12]  == 2 && $aRow[22]  == 0 ? '<a href="" class="btnAddTTicketLucky" rel="' . $aRow[0] . '" name="' . $aRow[2] . '"><img alt="Print Lucky Ticket" onmouseover="Tip(\'Add Lucky Ticket\')" src="' . $this->webroot . 'img/button/luck_ticket.png" style="width: 16px;" /></a> ' : '') .
            // ($allowPrintLucky && $aRow[20]  == 1 ? '<a href="" class="btnPrintTTicketLucky" rel="' . $aRow[0] . '" name="' . $aRow[2] . '"><img alt="Print Lucky Ticket" onmouseover="Tip(\'' . ACTION_PRINT . ' Lucky Ticket\')" src="' . $this->webroot . 'img/button/ticket-icon.png" style="width: 16px;" /></a> ' : '') .
            ($allowEdit && $aRow[12]  > 0 && $isOpenDate == 1 ? '<a href="" class="btnEditTTicket" rel="' . $aRow[0] . '" name="' . $aRow[2] . '"><img alt="Edit" onmouseover="Tip(\'' . ACTION_EDIT . '\')" src="' . $this->webroot . 'img/button/edit.png" /></a> ' : '') .
            ($allowCancel && $aRow[11]  == 2 && $aRow[12]  == 1 && ($timeNow < $timeJourney) ? '<a href="" class="btnCancelTTicket" rel="' . $aRow[0] . '" name="' . $aRow[2] . '"><img alt="Cancel" onmouseover="Tip(\'' . ACTION_CANCEL . '\')" src="' . $this->webroot . 'img/button/void.png" /></a> ' : '') .
            (($allowFullDelete && $aRow[12]  > 0 && $agencyPaid == false) || ($allowDelete && ($timeNow < $timeJourney) && $aRow[12]  > 0 && $agencyPaid == false) ? '<a href="" class="btnDeleteTTicket" rel="' . $aRow[0] . '" name="' . $aRow[2] . '"><img alt="Delete" onmouseover="Tip(\'' . ACTION_DELETE . '\')" src="' . $this->webroot . 'img/button/delete.png" /></a>' : '');
    $output['aaData'][] = $row;
}
