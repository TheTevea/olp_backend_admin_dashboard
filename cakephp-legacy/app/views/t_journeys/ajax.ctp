...
while ($aRow = mysql_fetch_array($rResult)) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        if ($i == 0) {
            ...
            $row[] = number_format($aRow[$i], 2)." ".$aRow[7];
        } else if ($aColumns[$i] == 't_journeys.foreigner_price') {
            $priceAddOn = "";
            // Check Default Price
            $sqlPD = mysql_query("SELECT * FROM t_journey_price_defaults WHERE offline_project_id = 1 AND destination_from_id = ".$aRow[17]." AND destination_to_id = ".$aRow[18]." AND t_transportation_type_id = ".$aRow[19]." AND status = 1 AND main_branch_id = ".$user['User']['main_branch_id']." ORDER BY id DESC LIMIT 1");
            if(mysql_num_rows($sqlPD)){
                $rowPD = mysql_fetch_array($sqlPD);
                $markUp = $rowPD['price'];
                $priceAddOn = "(Fixed)";
            } else {
                $sqlPDA = mysql_query("SELECT * FROM t_journey_price_defaults WHERE offline_project_id = 1 AND destination_from_id = ".$aRow[17]." AND destination_to_id = ".$aRow[18]." AND t_transportation_type_id = ".$aRow[19]." AND status = 1 AND (main_branch_id IS NULL OR main_branch_id = '') ORDER BY id DESC LIMIT 1");
            ...
            $sqlPA = mysql_query("SELECT * FROM t_journey_price_periods WHERE offline_project_id = 1 AND destination_from_id = ".$aRow[17]." AND destination_to_id = ".$aRow[18]." AND t_transportation_type_id = ".$aRow[19]." AND start <= '".date("Y-m-d")."' AND end >= '".date("Y-m-d")."' AND status = 1 AND (main_branch_id IS NULL OR main_branch_id = '') ORDER BY id DESC LIMIT 1");
            ...
            $row[] = number_format($markUp, 2)." ".$aRow[7].' <span style="color: blue;">'.$priceAddOn.'</span>';
        } else if ($aColumns[$i] == 't_journeys.agent_price_amount') {
            $row[] = number_format($price, 2)." ".$aRow[7];
    ...
    $row[] =
            ...
            ($allowView && $status > 0 ? '<a href="#" class="btnUpdateNoteTJourney" rel="' . $aRow[0] . '" name="' . $aRow[1] . '" note="' . $aRow[20] . '"><img alt="'.ACTION_UPDATE_NOTE.'" onmouseover="Tip(\'' . ACTION_UPDATE_NOTE . '\')" src="' . $this->webroot . 'img/button/note.png" style="width: 16px;" /></a> ' : ' ') .
            ...
            // ($allowCloneJourney && $status > 0 && SERVER_TYPE == 1 ? '<a href="#" class="btnCloneTJourney" rel="' . $aRow[14] . '" name="' . $aRow[5] . '"><img alt="'.TABLE_CLONE_JOURNEY.'" onmouseover="Tip(\'' . TABLE_CLONE_JOURNEY . '\')" src="' . $this->webroot . 'img/button/clone_journey.png" style="width: 16px;" /></a> ' : ' ') .
