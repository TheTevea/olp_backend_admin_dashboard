...
<div id="<?php echo $printArea; ?>">
    ...
    $totalMarkup = 0;
    $totalBonus  = 0;
    $sqlTicket = mysql_query("SELECT id, (IFNULL(t_tickets.total_amount, 0) - IFNULL(t_tickets.discount_amount, 0)) AS total_amount, total_vat, t_agent_id, t_journey_id, IFNULL(t_tickets.total_markup, 0) AS total_markup, IFNULL(t_tickets.commission, 0) AS commission, IFNULL(t_tickets.total_bonus, 0) AS total_bonus, t_tickets.total_seat, t_tickets.date FROM t_tickets WHERE ".$condition);
    while($rowTicket = mysql_fetch_array($sqlTicket)){
        ...
        // if($rowAgency['type'] == 3){ // APi
            if($rowAgency['commission_type'] == 1){ // Set Percentage
                $totalCommission += replaceThousand(number_format((($rowTicket['total_amount'] + $rowTicket['total_vat']) * $rowAgency['commission']) / 100, 2));
                ...
                $totalCommission += replaceThousand(number_format($commission, 2));
            } else { // Fixed Amount
                $totalCommission += replaceThousand(number_format($rowAgency['commission'], 2));
            }
        // } else {
    ...
    $sqlTicket3M = mysql_query("SELECT id, (IFNULL(t_tickets.total_amount, 0) - IFNULL(t_tickets.discount_amount, 0)) AS total_amount, total_vat, t_agent_id, t_journey_id, IFNULL(t_tickets.total_markup, 0) AS total_markup, IFNULL(t_tickets.commission, 0) AS commission, IFNULL(t_tickets.total_bonus, 0) AS total_bonus, t_tickets.total_seat, t_tickets.date FROM t_ticket_3months AS t_tickets WHERE ".$condition);
    ...
    <table cellpadding="5" cellspacing="0" style="width: 100%;">
        ...
        <tr>
            ...
            <td style="width: 120px; font-size: 14px; font-weight: bold;"><?php echo number_format(($totalFare - $totalCommission - $totalBonus), 2); ?> $</td>
            <?php
            if($isApi == 0){
            ?>
