...
while ($aRow = mysql_fetch_array($rResult)) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        if ($i == 0) {
            ...
            // if($aRow[22] == 3){ // Api
                if($aRow[18] == 1){ // Percentage
                    $row[] = number_format(($aRow[10] * $aRow[21]) / 100, 2)." ".$aRow[16];
                    $excelContent .= "\t".number_format(($aRow[10] * $aRow[21]) / 100, 2);
                } else if($aRow[18] == 2) { // Agency Price
                    $row[] = number_format(($aRow[19] - $aRow[20]), 2)." ".$aRow[16];
                    $excelContent .= "\t".number_format(($aRow[19] - $aRow[20]), 2);
                } else { // Fixed Amount
                    $row[] = number_format($aRow[21], 2)." ".$aRow[16];
                    $excelContent .= "\t".number_format($aRow[21], 2);
                }
            // } else {
            //     $row[] = number_format($aRow[$i], 2)." ".$aRow[16];
            //     $excelContent .= "\t".number_format($aRow[$i], 2)." ".$aRow[16];
            // }   
        ...
        } else if ($aColumns[$i] == 't_agents.commission_type' || $aColumns[$i] == 't_journeys.unit_price' || $aColumns[$i] == 't_journeys.agent_price_amount' || $aColumns[$i] == 't_agents.type' || $aColumns[$i] == 't_agents.commission') {
        } else if ($aColumns[$i] != ' ') {
            /* General output */
            $row[] = $aRow[$i];
            $excelContent .= "\t".$aRow[$i];
