...
class TTicketsController extends AppController {
    ...
    function index() {
        ...
        // $tDestinationFroms = ClassRegistry::init('TDestination')->find('all', array("conditions" => array("TDestination.is_active = 1", 'TDestination.offline_project_id' => 1, "TDestination.id IN (SELECT t_destination_id FROM branch_destinations WHERE branch_id IN (SELECT branch_id FROM user_branches WHERE user_id = ".$user['User']['id']."))")));
        $tDestinationFroms = ClassRegistry::init('TDestination')->find('all', array("conditions" => array("TDestination.is_active = 1", 'TDestination.offline_project_id' => 1)));
    ...
    function add($journeyId, $departureId, $date, $editId = 0) {
        ...
        if (!empty($this->data) && !empty($journeyId) && !empty($departureId) && !empty($date)) {
            ...
            if(!empty($this->data['TTicket']['t_agent_id'])){
                $sqlAgency = mysql_query("SELECT commission, commission_type, payment, max_balance, `type`, apply_bonus, bonus FROM t_agents WHERE id = ".$this->data['TTicket']['t_agent_id']);
                if(mysql_num_rows($sqlAgency)){
                    ...
                    if($rowAgency['commission_type'] != 2){ // != Default Agency Price
                        if($rowAgency['commission_type'] == 1){ // Commission (%)
                            if($rowAgency['commission'] > 0 && $this->data['TTicket']['total_amount'] > 0){
                                $totalCommission = (($this->data['TTicket']['total_amount'] + $this->data['TTicket']['total_vat']) * $rowAgency['commission']) / 100;
                            ...
                            $totalCommission  = $rowAgency['commission'] * sizeof($this->data['seat_number']);
                            $this->data['TTicket']['commission'] = $this->Helper->replaceThousand(number_format($totalCommission, 2));
                    ...
                    if($rowAgency['type'] == 1 || $rowAgency['type'] == 2){ // Agency Type (Offline or Online or APi)
                        ...
                        if($rowAgency['payment'] == 1 || $rowAgency['payment'] == 2){ // Prepaid or Postpaid
                            if($rowAgency['commission_type'] == 2){ // Agency Price
                                // Check Foriegner Price
                                // if($this->data['TTicket']['price_type'] == 3){ // Foriegner
                                //     $totalAgencyPrice = $TJourney['TJourney']['agetn_price_percent'] * sizeof($this->data['seat_number']);
                                // } else {
                                ...
                                if(mysql_num_rows($sqlPJ)){
                                    ...
                                    // if($this->data['TTicket']['price_type'] == 3){ // Foriegner  
                                    //     $totalAgencyPrice  = $rowPJ['agency_price_foreigner'] * sizeof($this->data['seat_number']);
                                    // } else {
                                    ...
                                    if(mysql_num_rows($sqlPA)){
                                        ...
                                        if($rowPAPrice['price_type'] == 1){
                                            // if($this->data['TTicket']['price_type'] == 3){ // Foriegner  
                                            //     $totalAgencyPrice  = $rowPAPrice['agency_price_foreigner'] * sizeof($this->data['seat_number']);
                                            // } else {
                                                $totalAgencyPrice  = $rowPAPrice['agency_price'] * sizeof($this->data['seat_number']);
                                            // }
                                        } else {
                                            // if($this->data['TTicket']['price_type'] == 3){ // Foriegner
                                            //     $totalAgencyPrice  = $totalAgencyPrice + ($rowPAPrice['agency_price_foreigner'] * sizeof($this->data['seat_number']));
                                            // } else {
            ...
            if($avaiable == true && $checkBalance == true){
                ...
                if($this->TTicket->save($this->data)) {
                    ...
                    $result['total_dis']        = number_format($ticketDiscount, 2);
                    ...
                    $result['total_usd']        = number_format($result['total_amount'] - $ticketDiscount + $this->data['TTicket']['lucky_draw_fee'], 2);
    ...
    function viewSchedule($desFrom, $desTo, $date, $isReturn, $editId = 0){
        ...
        // $journeys = ClassRegistry::init('TJourney')->find('all', array('conditions' => array('TJourney.t_destination_from_id' => $desFrom, 'TJourney.t_destination_to_id' => $desTo, 'TJourney.status' => 1, 'TJourney.offline_project_id' => 1, $condition), "order" => array("TDepartureTime.name", "TTransportationType.name")));
        $journeys = ClassRegistry::init('TJourney')->find('all', array('conditions' => array('TJourney.t_destination_from_id' => $desFrom, 'TJourney.t_destination_to_id' => $desTo, 'TJourney.status' => 1, 'TJourney.offline_project_id' => 1, $condition), "order" => array("TDepartureTime.name", "TTransportationType.name")));
    ...
    function releaseSeat($id = null) {
        ...
        if(mysql_num_rows($sqlSeat)){
            ...
            // Update Ticket Detail
            mysql_query("UPDATE ".$tblTicketDetail." SET release_date = now(), release_by = ".$user['User']['id'].", is_sync = 1 WHERE id = ".$id);
            // Update History
