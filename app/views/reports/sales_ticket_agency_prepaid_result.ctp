...
<div id="<?php echo $printArea; ?>">
    ...
    $totalBonus  = 0;
    $totalSeat   = 0;
    $sqlTicket = mysql_query("SELECT id, t_tickets.date, IFNULL(t_tickets.total_amount, 0) AS total_amount, IFNULL(t_tickets.discount_amount, 0) AS discount_amount, IFNULL(t_tickets.total_vat, 0) AS total_vat, IFNULL(t_tickets.total_bonus, 0) AS total_bonus, t_agent_id, t_journey_id, IFNULL(t_tickets.total_markup, 0) AS total_markup, IFNULL(t_tickets.commission, 0) AS commission, t_tickets.journey_date FROM t_tickets WHERE ".$condition);
    while($rowTicket = mysql_fetch_array($sqlTicket)){
        ...
        $rowAgency = mysql_fetch_array($sqlAgency);
        $agePrice  = 0;
        if($rowAgency['commission_type'] == 1){ // Percentage
            $totalCommission += (($rowTicket['total_amount'] + $rowTicket['total_vat']) * $rowAgency['commission']) / 100;
