...
$aColumns = array(
    ...
    '(IFNULL(t_tickets.total_amount, 0) + IFNULL(t_tickets.total_vat, 0))',
...
while ($aRow = mysql_fetch_array($rResult)) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        if ($i == 0) {
            ...
            if($aRow[20] == 1){ // Percentage
                $row[] = number_format(($aRow[10] * $aRow[22]) / 100, 2)." ".$aRow[18];
                $excelContent .= "\t".number_format($commission, 2)." ".$aRow[18];
            } else if($aRow[20] == 2){ // Commission Type = Fixed Amount
                $row[] = number_format($commission, 2)." ".$aRow[18];
                $excelContent .= "\t".number_format($commission, 2)." ".$aRow[18];
            } else {
                $row[] = number_format($aRow[22], 2)." ".$aRow[18];
                $excelContent .= "\t".number_format($aRow[22], 2)." ".$aRow[18];
            }            
