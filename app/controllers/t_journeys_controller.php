...
class TJourneysController extends AppController {
    ...
    function ajax($from = 'all', $to = 'all', $status = 'all', $type = 'all', $checked = 'all') {
        $this->layout = 'ajax';
        $this->set(compact('from', 'to', 'status', 'type', 'checked'));
    }
        ...
        if (!empty($this->data)) {
            if ($this->Helper->checkDouplicate('description', 't_journeys', $this->data['TJourney']['description'], 'status > 0 AND company_id = '.$this->data['TJourney']['company_id'].' AND branch_id = '.$this->data['TJourney']['branch_id'])) {
                ...
                if($this->TJourney->save($this->data)) {
                    ...
                    //         mysql_query("INSERT INTO t_journey_agent_prices (t_journey_id,t_agent_id,amount,percent,created) VALUES ('".$lastInsertId."','".$_POST['agent_id'][$i]."','".$_POST['agent_amount'][$i]."','".$_POST['agent_percent'][$i]."','".$dateNow."')");
                    //     }
                    // }
                    ...
                    if(!empty($this->data['TJourney']['price_period_from']) && !empty($this->data['TJourney']['price_period_to'])){
                        mysql_query("INSERT INTO `t_journey_price_periods` (`id`, `sys_code`, `offline_project_id`, `t_journey_id`, `main_branch_id`, `name`, `destination_from_id`, `destination_to_id`, `t_transportation_type_id`, `start`, `end`, `price`, `foreigner_price`, `membership`, `agency_price`, `agency_price_foreigner`, `created`, `created_by`, `modified`, `modified_by`, `apply_to`, `price_type`, `apply_type`, `status`) 
                                     VALUES (NULL, NULL, '1', ".$lastInsertId.", NULL, NULL, NULL, NULL, NULL, '".$this->data['TJourney']['price_period_from']."', '".$this->data['TJourney']['price_period_to']."', '".$this->data['TJourney']['price_period_price']."', '".$this->data['TJourney']['price_period_foreigner_price']."', '".$this->data['TJourney']['price_period_membership']."', '".$this->data['TJourney']['price_period_agency_price']."', '".$this->data['TJourney']['price_period_agency_price_foreigner']."', NOW(), '".$user['User']['id']."', NULL, NULL, '1', '1', '1', '1');");
                    }
                    ...
                    if(!empty($this->data['TJourney']['price_period_from_internal']) && !empty($this->data['TJourney']['price_period_to_internal'])){
                        mysql_query("INSERT INTO `t_journey_price_periods` (`id`, `sys_code`, `offline_project_id`, `t_journey_id`, `main_branch_id`, `name`, `destination_from_id`, `destination_to_id`, `t_transportation_type_id`, `start`, `end`, `price`, `foreigner_price`, `membership`, `agency_price`, `agency_price_foreigner`, `created`, `created_by`, `modified`, `modified_by`, `apply_to`, `price_type`, `apply_type`, `status`) 
                                     VALUES (NULL, NULL, '1', ".$lastInsertId.", NULL, NULL, NULL, NULL, NULL, '".$this->data['TJourney']['price_period_from_internal']."', '".$this->data['TJourney']['price_period_to_internal']."', '".$this->data['TJourney']['price_period_price_internal']."', '".$this->data['TJourney']['price_period_foreigner_price_internal']."', 0, 0, 0, NOW(), '".$user['User']['id']."', NULL, NULL, '1', '1', '2', '1');");
                    }
                    // VET Air Bus Promotion
                    if(!empty($this->data['TJourney']['promotion_dis_start']) && !empty($this->data['TJourney']['promotion_dis_end'])){
                        $discount = 0;
                        $price = 0;
                        $priceFor = 0;
                        if(!empty($this->data['TJourney']['promotion_dis_percentage'])){
                            $discount = $this->data['TJourney']['promotion_dis_percentage'];
                        }
                        if(!empty($this->data['TJourney']['promotion_dis_price'])){
                            $price = $this->data['TJourney']['promotion_dis_price'];
                        }
                        if(!empty($this->data['TJourney']['promotion_dis_price_foreigner'])){
                            $priceFor = $this->data['TJourney']['promotion_dis_price_foreigner'];
                        }
                        mysql_query("INSERT INTO `t_journey_payment_promotions` (`id`, `t_journey_id`, `payment_method_id`, `discount`, `date_start`, `date_end`, `price`, `foreigner_price`, `type`, `created`, `created_by`) 
                                     VALUES (NULL, ".$lastInsertId.", '".$this->data['TJourney']['promotion_dis_paymethod']."', '".$discount."', '".$this->data['TJourney']['promotion_dis_start']."', '".$this->data['TJourney']['promotion_dis_end']."', '".$price."', '".$priceFor."', '".$this->data['TJourney']['promotion_dis_type']."', NOW(), '".$user['User']['id']."');");
                    }
                    // Agency APi Webhook
    ...
    function edit($id = null) {
        ...
        if (!empty($this->data)) {
            if ($this->Helper->checkDouplicateEdit('description', 't_journeys', $id, $this->data['TJourney']['description'], 'status > 0 AND company_id = '.$this->data['TJourney']['company_id'].' AND branch_id = '.$this->data['TJourney']['branch_id'])) {
                ...
                if($this->TJourney->Save($this->data)){ 
                    // Update History Info
                    // mysql_query("INSERT INTO t_journey_info_edit_boarding_points SELECT * FROM t_journey_boarding_points WHERE t_journey_id = ".$id);
                    ...
                    if(!empty($this->data['TJourney']['price_period_from']) && !empty($this->data['TJourney']['price_period_to'])){
                        mysql_query("INSERT INTO `t_journey_price_periods` (`id`, `sys_code`, `offline_project_id`, `t_journey_id`, `main_branch_id`, `name`, `destination_from_id`, `destination_to_id`, `t_transportation_type_id`, `start`, `end`, `price`, `foreigner_price`, `membership`, `agency_price`, `agency_price_foreigner`, `created`, `created_by`, `modified`, `modified_by`, `apply_to`, `price_type`, `status`) 
                                     VALUES (NULL, NULL, '1', ".$id.", NULL, NULL, NULL, NULL, NULL, '".$this->data['TJourney']['price_period_from']."', '".$this->data['TJourney']['price_period_to']."', '".$this->data['TJourney']['price_period_price']."', '".$this->data['TJourney']['price_period_foreigner_price']."', '".$this->data['TJourney']['price_period_membership']."', '".$this->data['TJourney']['price_period_agency_price']."', '".$this->data['TJourney']['price_period_agency_price_foreigner']."', NOW(), '".$user['User']['id']."', NULL, NULL, '1', '1', '1');");
                    }
                    // Price Period Internal
                    mysql_query("UPDATE t_journey_price_periods SET status = 0 WHERE t_journey_id = ".$id." AND apply_type = 2");
                    if(!empty($this->data['TJourney']['price_period_from_internal']) && !empty($this->data['TJourney']['price_period_to_internal'])){
                        mysql_query("INSERT INTO `t_journey_price_periods` (`id`, `sys_code`, `offline_project_id`, `t_journey_id`, `main_branch_id`, `name`, `destination_from_id`, `destination_to_id`, `t_transportation_type_id`, `start`, `end`, `price`, `foreigner_price`, `membership`, `agency_price`, `agency_price_foreigner`, `created`, `created_by`, `modified`, `modified_by`, `apply_to`, `price_type`, `apply_type`, `status`) 
                                     VALUES (NULL, NULL, '1', ".$id.", NULL, NULL, NULL, NULL, NULL, '".$this->data['TJourney']['price_period_from_internal']."', '".$this->data['TJourney']['price_period_to_internal']."', '".$this->data['TJourney']['price_period_price_internal']."', '".$this->data['TJourney']['price_period_foreigner_price_internal']."', 0, 0, 0, NOW(), '".$user['User']['id']."', NULL, NULL, '1', '1', '2', '1');");
                    }
                    // VET Air Bus Promotion
                    mysql_query("UPDATE t_journey_payment_promotions SET status = 0 WHERE t_journey_id = ".$id);
                    if(!empty($this->data['TJourney']['promotion_dis_start']) && !empty($this->data['TJourney']['promotion_dis_end'])){
                        $discount = 0;
                        $price = 0;
                        $priceFor = 0;
                        ...
                        mysql_query("INSERT INTO `t_journey_payment_promotions` (`id`, `t_journey_id`, `payment_method_id`, `discount`, `date_start`, `date_end`, `price`, `foreigner_price`, `type`, `created`, `created_by`) 
                                     VALUES (NULL, ".$id.", '".$this->data['TJourney']['promotion_dis_paymethod']."', '".$discount."', '".$this->data['TJourney']['promotion_dis_start']."', '".$this->data['TJourney']['promotion_dis_end']."', '".$price."', '".$priceFor."', '".$this->data['TJourney']['promotion_dis_type']."', NOW(), '".$user['User']['id']."');");
