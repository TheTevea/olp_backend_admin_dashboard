    ...
    function add() {
        ...
        if (!empty($this->data)) {
            ...
            if ($this->Helper->checkDouplicate('username', 'users', $this->data['User']['username'], $condition)) {
                ...
                $this->data['User']['offline_project_id'] = 1;
        ...
        if($user['User']['type'] != 1){
            $groups = ClassRegistry::init('Group')->find('list', array('conditions' => array('is_active' => '1', "(offline_project_id = 1 || id = 2)"), 'order' => 'name'));
        } else {
            $groups = ClassRegistry::init('Group')->find('list', array('conditions' => array('is_active' => '1'), 'order' => 'name'));
        }
        ...
        $this->set(compact('groups', 'mainBranches'));
    }
